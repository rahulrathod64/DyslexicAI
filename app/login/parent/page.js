...
export default function ParentLogin() {
  ...
  const handleSubmit = async () => {
    try {
      await axios.post('/appi/login/parent', form);
  ...
  return (
    <div className="min-h-screen bg-black text-white flex items-center justify-center px-4">
      <div className="bg-zinc-900 p-10 rounded-2xl shadow-2xl max-w-md w-full">
        <h1 className="text-3xl font-bold text-purple-500 mb-6 text-center">Parent Login</h1>
        ...
        <p className="mt-6 text-center text-sm text-gray-400">
          ...
          <a href="/signup/parent" className="text-purple-500 hover:underline">Sign Up</a>
        </p>
      </div>
    </div>
  );
